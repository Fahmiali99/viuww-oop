...
(function ($) {
    $(document).ready(function () {
        ...
        //Shows updated notifikasi popup 
        var updateNotification = function (task, notificationText, newClass) {
            ...
            // Jika kelas khusus disediakan untuk munculan, tambahkan itu
            if (newClass)
                notificationPopup.addClass(newClass);
            // Jika sudah ada waktu tunggu untuk menyembunyikan munculan saat ini, kosongkan.
            if (notificationTimeout)
                clearTimeout(notificationTimeout);
            // Init timeout untuk menyembunyikan popup setelah 3 detik
        ...
        var addTask = function () {
            ...
            if (newTask == '') {
                ...
                // Show notification
                updateNotification(newTask, 'Tambah ke daftar');
        ...
        $('#todo-list').on('click', '.task-action-btn .delete-btn', function () {
            ...
            task.remove();
            updateNotification(taskText, ' Sudah di hapus.');
        });
        ...
        $('#todo-list').on('click', '.task-action-btn .complete-btn', function () {
            var task = $(this).closest('.task');
            var taskText = task.find('.task-label').text();
            var newTitle = task.hasClass('completed') ? 'Tandai Selesai' : 'Tandai Belum Selesai';
            $(this).attr('title', newTitle);
            task.hasClass('completed') ? updateNotification(taskText, 'ditandai sebagai Belum Selesai.') : updateNotification(taskText, ' ditandai sebagai selesai.', 'sukses');
        ...
        $('#mark-all-finished').click(function () {
            ...
            updateNotification('Semua teks', 'ditandai sebagai selesai.', 'sukses');
        });
        ...
        $('#mark-all-incomplete').click(function () {
            ...
            $('#mark-all-finished').removeClass('move-up');
            updateNotification('Semua teks', 'ditandai sebagai Belum Selesai.');
        });

        $(".clear-all").click(function () {
            $(".task").empty();
            updateNotification('Semua data terhapus');
        });
    });
}(jQuery))